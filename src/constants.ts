...
export const ErrorTips: Record<ErrorType, string> = {
  [ErrorType.NO_SELECT_RECORD]: '当前没有选中单元格，请选中一个单元格后使用',
  [ErrorType.FIELD_TYPE_NOT_SUPPORT]: '当前单元格所在的字段不支持', // 针对字段类型不支持的
  [ErrorType.NO_URL_CONTENT]: '当前单元格的内容为空，请重写单元格内容或选择其他单元格预览', // 针对单元格没有内容
  [ErrorType.URL_FORMAT_ERROR]: '当前单元格没有 URL 或 URL 不符合格式要求，请重写单元格内容或选择其他单元格预览', //针对单元格有内容但是没有可以解析的url
}
...
import { bitable, FieldType, IOpenCellValue, IOpenDocumentMentionSegment, IOpenSegment, IOpenSegmentType, IOpenUrlSegment } from '@base-open/web-api';
import { ErrorType, UrlRegExp } from './constants';
 ...
 * 根据字段类型，获取字段内容中的链接
 */
export function getFieldUrlValue(fieldType: FieldType, value: IOpenCellValue): UrlValueInfo {
  ...
  switch (fieldType) {
    ...
    case FieldType.Text: {
      ...
      (value as IOpenSegment[])?.some?.(item => {
        if (item.type === IOpenSegmentType.Url || item.type === IOpenSegmentType.Mention) {
          const url = (item as IOpenDocumentMentionSegment | IOpenUrlSegment).link || item.text;
    ...
    case FieldType.Url: {
      const url = (value as IOpenUrlSegment[])?.[0]?.link || (value as IOpenUrlSegment[])?.[0]?.text;
      if (!url) {

...
export default function App() {
  ...
  const [errorType, setErrorType] = useState<ErrorType | string>(ErrorType.NO_SELECT_RECORD);
  ...
  return (
    <main>
      {url ? <Preview onLoad={() => setLoading(false)} url={url} /> : <div className="content-tips-container">
        <div className="content-tips">
          <DisabaledWeb theme="outline" size="36" fill="#333" />
          <span className="content-tips-text">{errorType ? ErrorTips[errorType as ErrorType] : "无可预览的内容"}</span>
        </div>
      </div>}
